...
def revive_code_task(job_id, git_repo_base, git_repo_old, workdir):
    ...
    try:
        ...
        is_env_setup_correctly = utils.setup_r_base_environment(agent, workdir, global_context, stream_callback)
        ...
        is_env_setup_correctly = utils.setup_r_old_environment(agent, git_repo_old, workdir, global_context, stream_callback)
        if not is_env_setup_correctly:
            job_status[job_id]['status'] = 'failed'
        ...
        is_dependencies_resolved = utils.resolve_dependencies(agent, git_repo_base, git_repo_old, workdir, global_context, stream_callback)
        ...
        streaming_queues[job_id].put("\n\n=== Final verification ===\n")
        base_setup_correct, old_setup_correct = utils.verify_complete_integration(agent, git_repo_base, git_repo_old, workdir, global_context, stream_callback)
        
        if not base_setup_correct or not old_setup_correct:
            job_status[job_id]['status'] = 'failed'
            job_status[job_id]['error'] = f"Verification failed - Base: {base_setup_correct}, Old: {old_setup_correct}"
            streaming_queues[job_id].put(None)  # Signal end
            return
...
def submit_job():
    ...
    thread = threading.Thread(
        target=revive_code_task,
        args=(job_id, r_base, r_old, workdir)
    )
    thread.daemon = True
    thread.start()
