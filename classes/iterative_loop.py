...
class IterativeLoop:
    ...
    @weave.op()
    def activate_env_and_run_script(
        self, env_name: str = "env_r_base", script_path: str = "run test_old.sh"
    ):
...
"""
            ...
            # Run the prompt through the Cursor CLI agent with Weave tracking
            with weave.attributes(
                ...
                response_dict = self.agent.run_prompt(prompt, timeout=self.timeout)

            # Extract response text from the dict
            ...
            print(
                "1. Make sure Cursor CLI is installed: curl https://cursor.com/install -fsS | bash"
            )
            print(
                f"2. Verify that the '{env_name}' conda environment exists: conda env list"
            )
    ...
    def fix_errors(self, error_details: str) -> bool:
        ...
        print("\n" + "!" * 80)
        print(
            f"ITERATION {self.iteration_count}/{self.max_iterations}: Errors detected, attempting to fix..."
        )
        print("!" * 80)
...
"""
        ...
        try:
            with weave.attributes(
                ...
                response_dict = self.agent.run_prompt(fix_prompt, timeout=self.timeout)
